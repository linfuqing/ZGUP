...
namespace ZG
{
    ...
    public partial class AssetManager
    {
        ...
        /*public IEnumerator GetOrDownloadSingle(
            string url,
            string name,
            ...
            DownloadHandler handler)
        {
            ...
            yield return assetManager.__Load(url, null, true);

            AssetInfo? info = null;
            if (assetManager.__assets != null)
            {
                Asset destination;
                if (assetManager.__assets.TryGetValue(name, out destination))
                {
                    Asset source;
                    if (__assets == null || !__assets.TryGetValue(name, out source) || source.data.info.version < destination.data.info.version)
                        info = destination.data.info;
                }
            }
            ...
            if (confirm != null)
                yield return confirm(info.Value.size);
            ...
            using (WebClient webClient = new WebClient())
            {
                ...
                webClient.DownloadProgressChanged += delegate (object sender, DownloadProgressChangedEventArgs e)
                {
                    if (e != null)
                    {
                        totalBytesToReceive = (ulong)e.TotalBytesToReceive;
                        bytesToReceive += (uint)e.BytesReceived;
                        progress = bytesToReceive * 1.0f / e.TotalBytesToReceive;
                    }
                };
                ...
                {
                    bytes = e == null ? new byte[0] : e.Result;
                };

                webClient.DownloadDataAsync(new Uri(url + name));

                while (bytes == null)
                {
                    handler(name, progress, bytesToReceive, bytesToReceive, totalBytesToReceive, 0, 1);

                    yield return null;
                }

                bytesToReceive = (uint)bytes.Length;
                handler(name, 1.0f, bytesToReceive, bytesToReceive, totalBytesToReceive, 0, 1);

                AssetData data;
                data.info = info.Value;
                data.type = AssetType.Compressed;
                data.fileOffset = 0;
                data.filePath = null;
                data.dependencies = null;
                Create(name, data, bytes, 0, bytesToReceive);
            }
        }
            ...
            DownloadHandler handler)
        {
            ...
            yield return assetManager.__Load(url, folder, true);
            ...
            if (__assets != null)
            {
                List<string> names = new List<string>(__assets.Keys);
                foreach (string name in names)
                {
                    if (string.IsNullOrEmpty(folder) ? !string.IsNullOrEmpty(Path.GetDirectoryName(name)) : folder != Path.GetDirectoryName(name))
                        continue;

                    if (assetManager.__assets == null || !assetManager.__assets.ContainsKey(name))
                        Delete(name);
                }
            }
            ...
            Asset source;
            string assetName;
            List<(string, AssetData)> assets = null;
            foreach (KeyValuePair<string, Asset> pair in assetManager.__assets)
            {
                assetName = pair.Key;
                ...
                if (__assets == null || !__assets.TryGetValue(assetName, out source) || source.data.info.version < destination.info.version)
                {
                    ...
                    if (assets == null)
                        assets = new List<(string, AssetData)>();

                    assets.Add((assetName, destination));
                }
            }

            int numAssets = assets == null ? 0 : assets.Count;
            if (numAssets < 1)
                yield break;
            ...
            if (!string.IsNullOrEmpty(folder))
            {
                int length = url.Length, count = folder.Length;
                if (length > count)
                {
            ...
            ulong downloadedBytes, totalBytesDownload = 0;
            (string, AssetData) temp;
            byte[] data, md5hash;
            using (var md5 = new MD5CryptoServiceProvider())
            {
                ...
                {
                    temp = assets[i];
                    assetName = temp.Item1;
                    destination = temp.Item2;
                    while (true)
                    {
                        using (var www = UnityWebRequest.Get(url + assetName))
                        {
                            if (www == null)
                                yield break;
                            ...
                            totalBytesDownload += www.downloadedBytes;
                            ...
                            if (assetManager.version > 0)
                            {
                                if (!MemoryEquals(md5hash, destination.info.md5))
                                    continue;
                            }
                            else
                                destination.info.md5 = md5hash;

                            Create(assetName, destination, data, 0, (uint)www.downloadedBytes);
                        }
            ...
            foreach (var result in results)
            {
                ...
                for (i = 0; i < assetCount; ++i)
                {
                    ...
                    if (__assets == null || 
                        ...
                        (source.data.isReadOnly && (pack == null ? confirm == null : !pack.Contains(source.data.pack.name)) ? source.data.info.version - 1 : source.data.info.version) : //为了让废弃的包被替换
